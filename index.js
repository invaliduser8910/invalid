...
if (message.content.includes(invlnk)){let muterole=message.guild.roles.find(r=>r.name==="MUTED");message.author.addRole(muterole.id);bot.channels.get("674977831587020810").send(message.author.tag+" has been muted for advertising through basement\nMessage: "+message.content);return;}else {const attachment=new Attachment(a); bot.channels.get("647122545086169115").send(anonlnk); return;}}
