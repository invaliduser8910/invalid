...
if (message.content.includes(invlnk)){let mutee=message.guild.members.get('user ID');let muterole=message.guild.roles.find('name', "Self-Study-Shelter");mutee.addRole(muterole.id);bot.channels.get("674977831587020810").send(message.author.tag+" has been muted for advertising through basement\nMessage: "+message.content);return;}else {const attachment=new Attachment(a); bot.channels.get("647122545086169115").send(anonlnk); return;}
